...
function safeFileName(name: string) {
  const clean = String(name || "file").replace(/[^\w.\-]+/g, "-").replace(/-+/g, "-");
...
async function deleteFromDrive(fileId: string, accessToken: string, folderId: string) {
  ...
  if (!metadataRes.ok) {
    throw new Error(`Cannot inspect Google Drive file: ${metadata?.error?.message || metadataRes.statusText}`);
  }
  if (!Array.isArray(metadata.parents) || !metadata.parents.includes(folderId)) {
    throw new Error("Google Drive file is outside the configured upload folder");
  }
  ...
  if (!deleteRes.ok && deleteRes.status !== 404) {
    ...
    throw new Error(`Cannot delete Google Drive file: ${data?.error?.message || deleteRes.statusText}`);
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const folderName = safeFileName(String(form.get("folder") || "files"));
    if (!(file instanceof File)) return jsonResponse({ error: "Missing file" }, 400);
    const mimeType = file.type || "application/octet-stream";
    if (!/^(video|audio)\//.test(mimeType)) return jsonResponse({ error: "Only video/audio uploads are allowed" }, 400);
    if (file.size > 80 * 1024 * 1024) return jsonResponse({ error: "File is too large. Maximum is 80MB." }, 400);
    ...
    console.error(error);
    const message = error instanceof Error ? error.message : "Google Drive file operation failed";
