-- Fix MindUp learning messages + editable automatic message templates.
...
VALUES
  ('learning_notification', 'Tin nhắn học tập chung', '🔔 {{title}}

{{message}}

__ACTION__{"type":"url","label":"Mở chi tiết","url":"{{target_url}}"}', true, now()),
  ('session_score_notice', 'Điểm BTVN/Đề luyện tập trong buổi học', '📊 Điểm BTVN/Đề luyện tập lớp **{{class_name}}**

Học sinh: **{{student_name}}**
Bài/buổi: **{{lesson_name}}**
Ngày học: **{{session_date}}**
Điểm: **{{score}}/{{max_score}}**
{{note}}', true, now()),
  ('session_evaluation_notice', 'Nhận xét buổi học gửi phụ huynh', '📝 Nhận xét buổi học lớp **{{class_name}}** ngày **{{session_date}}**

Học sinh: **{{student_name}}**

{{message}}', true, now()),
  ('exam_score_notice', 'Điểm đề luyện tập sau khi chấm', '📊 {{title}}

{{message}}

__ACTION__{"type":"url","label":"Xem chi tiết","url":"{{target_url}}"}', true, now()),
  ('tuition_due_manual', 'Nhắc học phí chưa nộp thủ công', 'Bạn có học phí tháng {{month}} chưa thanh toán. Số tiền cần nộp là {{amount_label}}.', true, now()),
  ('tuition_partial_manual', 'Nhắc học phí còn thiếu thủ công', 'Bạn còn thiếu {{remaining_label}} học phí tháng {{month}}. Hãy kiểm tra và hoàn tất thanh toán.', true, now())
ON CONFLICT (id) DO UPDATE
SET
  name = EXCLUDED.name,
  content = COALESCE(public.message_templates.content, EXCLUDED.content),
  updated_at = now();

-- =========================================================
-- 2) Student/parent learning conversations
-- =========================================================
...
CREATE OR REPLACE FUNCTION public.can_access_learning_thread(
  p_student_id uuid,
...
GRANT EXECUTE ON FUNCTION public.list_student_learning_audiences(uuid) TO authenticated, service_role;
