...
// --- Final override for compact 2-line PDF answer editor ---
function getPdfQuestionDefaultsByType(type, count = 4) {
  ...
  if (type === "true_false") {
    return {
      ...
      partial_points: safeCount === 4 ? { 0: 0, 1: 0.1, 2: 0.25, 3: 0.5, 4: 1 } : { [safeCount]: 1 },
    };
  }
...
function withPdfPartialDefaults(question) {
  ...
  const partial = type === "essay" ? null : { ...(question.partial_points || defaults.partial_points || {}) };

  if (type === "multi_choice" || type === "short_answer") partial[count] = Number(question.points ?? defaults.points);
  ...
  if (type === "true_false" && count !== 4) partial[count] = partial[count] ?? Number(question.points ?? defaults.points);

  return {
    ...question,
    question_type: type,
    answer_count: type === "essay" ? 1 : count,
    points: Number(question.points ?? defaults.points),
    answer: question.answer ?? defaults.answer,
    partial_points: partial,
  };
}

function updatePdfDraftField(id, field, value) {
  PDF_STATE.draftQuestions = PDF_STATE.draftQuestions.map((q) => q.id === id ? withPdfPartialDefaults({ ...q, [field]: field === "points" ? Number(value || 0) : value }) : q);
...
function updatePdfDraftType(id, type) {
  ...
  PDF_STATE.draftQuestions = PDF_STATE.draftQuestions.map((q) => {
    ...
    return { ...q, question_type: normalized, answer_count: defaults.answer_count, answer: defaults.answer, points: defaults.points, partial_points: defaults.partial_points };
  });
  renderDraftQuestionList();
}
...
function updatePdfDraftAnswerCount(id, delta) {
  PDF_STATE.draftQuestions = PDF_STATE.draftQuestions.map((q) => {
    ...
    return withPdfPartialDefaults({
      ...
      partial_points: trimPdfPartialPoints(q.partial_points, q.question_type === "true_false" ? nextCount + 1 : nextCount),
    });
  });
  renderDraftQuestionList();
}
...
function renderInlinePdfAnswerEditor(q) {
  ...
  if (safe.question_type === "multi_choice") {
    ...
    return `<div class="draft-inline-row"><button class="icon-btn" type="button" onclick="updatePdfDraftAnswerCount('${safe.id}',-1)">-</button>${Array.from({ length: safe.answer_count }, (_, i) => {
      const key = String.fromCharCode(65 + i);
      return `<label class="answer-box ${selected.has(key) ? "active" : ""}"><input type="checkbox" ${selected.has(key) ? "checked" : ""} onchange="updatePdfDraftMC('${safe.id}','${key}',this.checked)"><span class="answer-box-key">${key}</span></label>`;
    }).join("")}<button class="icon-btn" type="button" onclick="updatePdfDraftAnswerCount('${safe.id}',1)">+</button></div>`;
  }
  ...
  if (safe.question_type === "short_answer") {
    const values = String(safe.answer || "").split(";").map((item) => item.trim());
    return `<div class="draft-inline-row"><button class="icon-btn" type="button" onclick="updatePdfDraftAnswerCount('${safe.id}',-1)">-</button>${Array.from({ length: safe.answer_count }, (_, i) => `<input class="input" style="width:72px" type="text" placeholder="${i + 1}" value="${escAttr(values[i] || "")}" oninput="updatePdfDraftShort('${safe.id}',${i},this.value)">`).join("")}<button class="icon-btn" type="button" onclick="updatePdfDraftAnswerCount('${safe.id}',1)">+</button></div>`;
  }
...
function renderInlinePdfPartialEditor(q) {
  const safe = withPdfPartialDefaults(q);
  if (safe.question_type === "essay") return `<div class="hint">Câu tự luận không có điểm chi tiết tự động.</div>`;
  const counts = safe.question_type === "true_false" ? Array.from({ length: safe.answer_count + 1 }, (_, i) => i) : Array.from({ length: safe.answer_count }, (_, i) => i + 1);
  return `<div class="draft-inline-row">${counts.map((count) => `<label style="display:flex;align-items:center;gap:4px"><span class="draft-inline-label">${count} ý</span><input class="input" style="width:58px" type="number" min="0" step="0.05" value="${safe.partial_points?.[count] ?? 0}" oninput="updatePdfDraftPartial('${safe.id}','${count}',this.value)"></label>`).join("")}</div>`;
}
      ...
      return `<div class="draft-answer-item" id="pdfDraftRow_${q.id}">
        <div class="draft-answer-compact">
          <div class="draft-answer-top">
            ...
            <div class="draft-inline-row"><span class="draft-inline-label">Loại câu:</span><select class="select" style="width:145px" onchange="updatePdfDraftType('${q.id}',this.value)"><option value="multi_choice" ${q.question_type === "multi_choice" ? "selected" : ""}>Trắc nghiệm</option><option value="true_false" ${q.question_type === "true_false" ? "selected" : ""}>Đúng / Sai</option><option value="short_answer" ${q.question_type === "short_answer" ? "selected" : ""}>Trả lời ngắn</option><option value="essay" ${q.question_type === "essay" ? "selected" : ""}>Tự luận</option></select></div>
            <div class="draft-inline-row"><span class="draft-inline-label">Đáp án:</span>${renderInlinePdfAnswerEditor(q)}</div>
            <button class="btn btn-danger btn-sm" type="button" onclick="deletePdfQuestion('${q.id}')">Xóa</button>
          </div>
          <div class="draft-answer-bottom">
            <div class="draft-inline-row"><span class="draft-inline-label">Điểm câu:</span><input class="input" style="width:64px" type="number" min="0" step="0.05" value="${q.points ?? 0}" oninput="updatePdfDraftField('${q.id}','points',this.value)"></div>
            <div><span class="draft-inline-label">Điểm chi tiết:</span>${renderInlinePdfPartialEditor(q)}</div>
          </div>
        </div>
      </div>`;
    }).join("")
          ...
          <div class="draft-answer-head">
            <div class="draft-answer-title">
              ...
              <div>
                ...
                <div class="hint">Chỉnh trực tiếp tại dòng này</div>
              </div>
            </div>
            <button class="btn btn-danger btn-sm" type="button" onclick="deletePdfQuestion('${q.id}')">Xóa</button>
          </div>
...
// Final override: keep PDF answer editing fully inline inside the create/edit exam modal.
...
// EOF override: remove any remaining "Sửa" button inside answer rows.
function renderDraftQuestionList() {
  PDF_EL.draftQuestionList.className = "draft-answer-list";
  PDF_EL.draftQuestionList.innerHTML = PDF_STATE.draftQuestions.length
    ? PDF_STATE.draftQuestions
