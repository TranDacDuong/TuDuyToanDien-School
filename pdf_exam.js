...
function renderInlinePdfAnswerEditor(question) {
  ...
  if (question.question_type === "multi_choice") {
    ...
    return `<div class="draft-inline-group"><span class="draft-inline-label">Đáp án:</span><div class="count-stepper"><button class="icon-btn" type="button" title="Giảm số đáp án" aria-label="Giảm số đáp án" onclick="updatePdfDraftAnswerCount('${question.id}',-1)">-</button><button class="icon-btn" type="button" title="Tăng số đáp án" aria-label="Tăng số đáp án" onclick="updatePdfDraftAnswerCount('${question.id}',1)">+</button></div>${Array.from({ length: question.answer_count }, (_, idx) => { const key = String.fromCharCode(65 + idx); return `<label class="draft-inline-option"><input type="checkbox" ${selected.has(key) ? "checked" : ""} onchange="updatePdfDraftMC('${question.id}','${key}',this.checked)"><span>${key}</span></label>`; }).join("")}</div>`;
  }
  if (question.question_type === "true_false") {
    const parsed = parsePdfTrueFalse(question.answer, question.answer_count || 4);
    return `<div class="draft-inline-group"><span class="draft-inline-label">Đáp án:</span><div class="count-stepper"><button class="icon-btn" type="button" title="Giảm số đáp án" aria-label="Giảm số đáp án" onclick="updatePdfDraftAnswerCount('${question.id}',-1)">-</button><button class="icon-btn" type="button" title="Tăng số đáp án" aria-label="Tăng số đáp án" onclick="updatePdfDraftAnswerCount('${question.id}',1)">+</button></div>${Array.from({ length: question.answer_count }, (_, idx) => { const key = String.fromCharCode(97 + idx); const state = parsed[key] || "Đ"; return `<button class="tf-state ${state === "Đ" ? "correct" : "wrong"}" type="button" onclick="updatePdfDraftTF('${question.id}','${key}')">${key.toUpperCase()}</button>`; }).join("")}</div>`;
  }
  const values = String(question.answer || "").split("|");
  return `<div class="draft-inline-group"><span class="draft-inline-label">Đáp án:</span><div class="count-stepper"><button class="icon-btn" type="button" title="Giảm số đáp án" aria-label="Giảm số đáp án" onclick="updatePdfDraftAnswerCount('${question.id}',-1)">-</button><button class="icon-btn" type="button" title="Tăng số đáp án" aria-label="Tăng số đáp án" onclick="updatePdfDraftAnswerCount('${question.id}',1)">+</button></div>${Array.from({ length: question.answer_count }, (_, idx) => `<input class="input draft-inline-text" type="text" value="${escAttr(values[idx] || "")}" oninput="updatePdfDraftShort('${question.id}',${idx},this.value)" placeholder="Ý ${idx + 1}">`).join("")}</div>`;
}
