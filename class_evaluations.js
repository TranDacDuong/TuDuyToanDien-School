(function () {
  ...
  function applyVariables(content, student) {
    const rawParent = (state.parents.get(student.id) || "").trim();
    const values = {
      ten_hoc_sinh: student.full_name || "học sinh",
      ten_phu_huynh: rawParent,
    ...
    result = result.replace(/Kính gửi anh\/chị\s*,/gi, "Kính gửi Quý phụ huynh,").replace(/\s{2,}/g, " ");
    return result.trim();
  }
