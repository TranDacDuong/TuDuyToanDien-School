...
(function () {
  ...
  function renderExamUI(overlay, examTitle, durationMin) {
    ...
    overlay.innerHTML =
      ...
      <div style="flex:1;display:flex;flex-direction:${isCompactMobile ? 'column' : 'row'};overflow:hidden;min-height:0">
        ...
        ${isCompactMobile ? `<div style="padding:10px 12px;background:var(--white);border-bottom:1px solid var(--border);display:flex;align-items:center;gap:10px">
          <button onclick="toggleClassExamNav()" style="background:#eff6ff;color:var(--navy);border:1px solid #bfdbfe;padding:9px 12px;border-radius:10px;font-size:.86rem;font-weight:700;cursor:pointer;font-family:var(--font-body)">Danh sách câu</button>
          <div style="font-size:.8rem;color:var(--ink-mid);line-height:1.5">Chạm để chuyển nhanh giữa các câu.</div>
        </div>` : ''}
        <div id="classExamMainArea" style="flex:1;overflow-y:auto;padding:${isCompactMobile ? '14px 12px 20px' : '18px 20px'};background:#f8fafc">${sectionsHtml}</div>
      </div>`;
    ...
    const mainArea = document.getElementById("classExamMainArea");
